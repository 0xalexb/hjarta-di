...
- Available middlewares:
  ...
  - `CORS(opts ...CORSOption)` - configurable CORS with preflight handling, hostname-based origin matching, wildcard support, credentials flag; uses functional options pattern with sensible defaults (origins `["*"]`, methods `["GET","HEAD","POST"]`, headers `["Origin","Accept","Content-Type","X-Requested-With"]`, maxAge 3600, no credentials); options: `WithAllowedOrigins(origins...)` sets bare hostnames (e.g., "example.com" not "https://example.com"), `WithAllowedMethods(methods...)`, `WithAllowedHeaders(headers...)`, `WithExposedHeaders(headers...)`, `WithMaxAge(seconds)`, `WithAllowCredentials()`, `WithOriginValidators(validators...)`; incoming Origin headers matched by extracting hostname; `ValidateHostname()` convenience returns all validators (no scheme, no path, no port, no wildcard, not empty); if AllowCredentials is true with only wildcard origins, disables credentials and logs slog.Warn
